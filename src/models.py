...
from .components import FluidComponent, ThermalLoad
...
class BuildingParams:
    """
    Parameters for large commercial building thermal model.
    Scaled for a university/corporate building (~10,000 m²).
    """
    floor_area: float = 10000.0        # m²
    thermal_mass: float = 500000.0     # kg (heavy concrete construction)
    specific_heat: float = 1000.0      # J/(kg·K) - effective
    UA_envelope: float = 3500.0        # W/K — building heat loss coefficient
    internal_gains: float = 150000.0   # W (150 kW — occupants, lighting, equipment)
    setpoint_temp: float = 21.0        # °C
    deadband: float = 1.0              # °C
    solar_aperture: float = 500.0      # m² — effective window area (~15% of facade)
    solar_gain_factor: float = 0.4     # SHGC — commercial low-e glass
...
class BuildingThermalMass(FluidComponent):
    ...
    def calculate_heating_demand(self) -> float:
        ...
        if self.heating_active:
            ...
            # Simple proportional control (scaled for commercial building)
            K_p = 50000.0  # W/K - proportional gain
    ...
    def calculate_outlet_temp(self, T_inlet: float, flow_rate: float, dt: float) -> float:
        ...
        fluid_cp = 3700.0  # water-glycol mixture
