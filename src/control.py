"""
Control system for solar-assisted heat pump (SAHP) system.
Manages solar loop pump and heat pump enable based on system state.
"""
...
class SAHPController(Controller):
    """
    Controller for solar-assisted heat pump system.

    Solar loop control: differential temperature with hysteresis
    (same proven approach as BasicController). Tank max temp is lower
    (~45°C) since the tank serves as a heat pump source, not direct heating.

    Heat pump control: enables heat pump when the building has heating
    demand and the tank temperature is above the minimum source threshold.
    """

    def __init__(
        self,
        name: str = "SAHPController",
        solar_dT_threshold: float = 5.0,   # °C — minimum dT to run solar pump
        tank_max_temp: float = 45.0,        # °C — SAHP source tanks run cooler
        hp_min_source_temp: float = -10.0,  # °C — heat pump low-limit
    ):
        super().__init__(name)
        self.solar_dT_threshold = solar_dT_threshold
        self.tank_max_temp = tank_max_temp
        self.hp_min_source_temp = hp_min_source_temp

        # Control state
        self.solar_pump_on = False
        self.hp_enable = False

    def compute_control(self, system_state: Dict[str, Any]) -> Dict[str, Any]:
        """
        Compute control actions for SAHP system.

        System state expected:
            - T_collector: Solar collector temperature (°C)
            - T_tank: Storage tank temperature (°C)
            - Q_demand: Building heating demand (W)
            - irradiance: Solar irradiance on collector (W/m²)
        """
        T_collector = system_state.get('T_collector', 20.0)
        T_tank = system_state.get('T_tank', 20.0)
        Q_demand = system_state.get('Q_demand', 0.0)
        ...
        # === Solar loop pump control (differential temperature + hysteresis) ===
        dT = T_collector - T_tank
        ...
        # === Heat pump enable control ===
        if Q_demand > 0 and T_tank > self.hp_min_source_temp:
            self.hp_enable = True
        else:
            self.hp_enable = False
