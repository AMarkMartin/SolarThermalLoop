...
def main():
    ...
    # Sized for ~20-30% solar fraction
    collector = SolarCollector(
        "SolarCollector",
        SolarCollectorParams(
            area=4.0,  # Modest system for supplemental heating
            efficiency=0.75,
            heat_loss_coef=4.0
        )
    )
    ...
    tank = StorageTank(
        "StorageTank",
        StorageTankParams(
            volume=0.3,  # 300 liters
            mass=300.0,
            surface_area=2.0,
            heat_loss_coef=1.2  # Better insulation
        ),
        initial_temp=35.0
    )

    # Building load - simplified but realistic
    building = SimplifiedBuildingLoad(
        "CommercialBuilding",
        floor_area=1000.0,
        design_heat_load=15000.0,  # 15 kW at design
    ...
    # Controller
    controller = BasicController(
        solar_dT_threshold=6.0,  # Lower threshold for winter
        tank_max_temp=75.0
    )
