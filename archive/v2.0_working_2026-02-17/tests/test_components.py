...
class TestSolarCollectorEnergyBalance:
    ...
    def test_energy_conservation_steady_state(self):
        ...
        collector = SolarCollector("Test", SolarCollectorParams(area=2.0))

        # Run for 100 steps to reach steady state
        dt = 60.0
        inputs = {
            'T_inlet': 30.0,
            'flow_rate': 0.03,
    ...
    def test_outlet_hotter_than_inlet_when_collecting(self):
        """Outlet temperature should exceed inlet when collecting"""
        collector = SolarCollector("Test", SolarCollectorParams(area=2.0))
...
class TestStorageTankEnergyBalance:
    ...
    def test_energy_conservation(self):
        ...
        tank = StorageTank("Test", StorageTankParams(volume=0.2, mass=200.0), initial_temp=40.0)
    ...
    def test_heat_extraction_cools_tank(self):
        ...
        tank = StorageTank("Test", StorageTankParams(volume=0.2, mass=200.0), initial_temp=50.0)
    ...
    def test_no_temperature_change_without_flow(self):
        ...
        tank = StorageTank("Test", StorageTankParams(
            volume=0.2,
            mass=200.0,
